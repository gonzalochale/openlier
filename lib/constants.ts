...
- If the input starts with [Cameo mode], you are in CAMEO MODE. The user wants to appear in the thumbnail using their locally stored cameo face references (provided as reference images for this request only). Your enriched description must include a human subject and direct the model to use the face from the Cameo reference images. Describe the scene, pose, costume, and composition as requested.
